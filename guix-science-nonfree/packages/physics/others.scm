...
(define-module (guix-science-nonfree packages physics others)
       ...
       (replace 'configure
		...
		(lambda* (#:key outputs (configure-flags '()) native-inputs inputs
			 ...
			 (let ((out (assoc-ref outputs "out"))
			       (PYTHON_DIR (assoc-ref %build-inputs "python@3")))
			   (format #t "build directory: ~s~%" (getcwd))
			   (format #t "configure flags: ~s~%" configure-flags)
			   (format #t "output directory: ~s~%" out)
			   ;; Node's configure script expects the CC
			   ...
			   (apply invoke
				  (string-append PYTHON_DIR "/bin/python3")
				  "configure.py"
  ...
  (package
   ...
   (inputs
    `(("bash" ,bash)
      ...
      ("gzip" ,gzip)
      ("gfortran" ,gfortran-8)
      ;; ("gfortran-toolchain" ,gfortran-toolchain-8)
      ))
   (arguments
    `(#:tests? #f
   ...
   (arguments
	...
	(lambda*
	 ...
	 (let* ((DIR (assoc-ref outputs "out"))
		...
		(STOMP "/site-packages/stomp") 
		(INSTALL (string-append DIR "/lib/python" PYTHON_VERSION STOMP)))
       ...
       (delete 'build) ;; "no setup.py found"
       (delete 'install) ;; "no setup.py found" 
       (delete 'check) ;; "no setup.py found" 
